...
window.showCreateClubPopup = async function () {
  const { value: formValues } = await Swal.fire({
    ...
    preConfirm: () => {
      ...
      if (!name) {
        Swal.showValidationMessage("กรุณาระบุชื่อชุมนุม");
        return false;
      }
  ...
  if (formValues) {
    ...
    const { data: year } = await db
      ...
      .single();
    const { data: user } = await db.auth.getUser();
    const { data: newClub, error } = await db
      .from("clubs")
      .insert({
    ...
    await db
      .from("club_teachers")
      .insert({ club_id: newClub.id, user_id: user.user.id, is_owner: true });
    Swal.fire({
      title: "สำเร็จ!",
      icon: "success",
      timer: 1500,
...
window.showEditClubPopup = async function (id, currentName, currentDesc) {
  const { value: formValues } = await Swal.fire({
    ...
    preConfirm: () => {
      ...
      if (!name) {
        Swal.showValidationMessage("ชื่อชุมนุมห้ามว่าง");
        return false;
      }
  ...
  if (formValues) {
    ...
    await db
      .from("clubs")
      .update({ name: formValues.name, description: formValues.desc })
      .eq("id", id);
    Swal.fire({
      title: "อัปเดตสำเร็จ!",
      icon: "success",
